...
names(vals) <- args
cix=c(vals[["a1"]], vals[["a2"]], vals[["d1"]], vals[["d2"]])
...
while(length(line <- readLines(input, n=1)) > 0){
  ...
  if(tf){
    write(paste(line, "NAs", sep="\t"), stderr())
    next
  }
  ...
  if(tf){
    write(paste(line, "notGCTA", sep="\t"), stderr())
    next
  }
  # Check for homozygote variants (should not happen in real data)
  tf <- el[1] == el[2]
  if(tf){
    write(paste(line, "hom", sep="\t"), stderr())
    next
  }
  tf <- el[3] == el[4]
  if(tf){
  ...
  if(tf1|tf2){
    write(paste(line, "palindrom", sep="\t"), stderr())
    next
  }
  ...
  if(tf3){
    write(paste(line, "notExpA2", sep="\t"), stderr())
    next
  }
  ...
  if(!tf4){
    write(paste(line, "notPossible", sep="\t"), stderr())
    next
  }
  ...
  write(paste(el[3], el[4], em, sep="\t"), stdout())
}
